...
function predictNextNum(line) {
  ...
  for (let i = matricSize - 1; i - 1 >= 0; i--) {
    ...
    let firsttNumOne = multiNumArray[i][0];
    let firstNumTwo = multiNumArray[i - 1][0];
    //   let lastNumOne = multiNumArray[i - 1][multiNumArray[i - 1].length - 1];
    //   let lastNumTwo = multiNumArray[i][multiNumArray[i].length - 1];
    multiNumArray[i - 1].unshift(firstNumTwo - firsttNumOne);
  }

  // Return the first number of the first num array
  return multiNumArray[0][0];
}
