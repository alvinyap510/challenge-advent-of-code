// Part2's question is a trap
// Basically there are 1 billion + seeds to compare
// An array is not sufficient to store the seeds
...
function extractSeed(linesArray) {
  ...
  for (let i = 0; i < intermediateArr.length; i += 2) {
    for (let k = 0; k < parseInt(intermediateArr[i + 1]); k++) {
      seedArr.push(parseInt(intermediateArr[i]) + k);
    }
  }
  console.log("Seed Array: ", seedArr);
  return seedArr;
}
